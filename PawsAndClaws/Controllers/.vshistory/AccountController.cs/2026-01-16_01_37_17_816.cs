...
namespace PawsAndClaws.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Profile()
        {
            ...
            var ownedPets = await _db.OwnedPets
                .AsNoTracking()
            ...
            var vm = new UserProfileViewModel
            {
                ...
                OwnedPets = ownedPets,
                CurrentPetCount = ownedPets.Count
            };
        ...
        // add current pet
        public async Task<IActionResult> AddOwnedPet([Bind(Prefix = "NewPet")] AddPetViewModel model)
        {
            var userId = _userManager.GetUserId(User);
            if (string.IsNullOrWhiteSpace(userId)) return RedirectToAction("Login");

            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = "Please fill in the pet details correctly.";
                return RedirectToAction("Profile", new { edit = 1 });
            }
            ...
            _db.OwnedPets.Add(pet);
            await _db.SaveChangesAsync();

            TempData["SuccessMessage"] = "Pet added.";
            return RedirectToAction("Profile", new { edit = 1 }); // optional
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> RemoveOwnedPet(int id)
        {
            var userId = _userManager.GetUserId(User);
            if (string.IsNullOrWhiteSpace(userId)) return RedirectToAction("Login");

            var pet = await _db.OwnedPets.FirstOrDefaultAsync(p => p.Id == id && p.UserId == userId);
            if (pet == null) return NotFound();

            _db.OwnedPets.Remove(pet);
            await _db.SaveChangesAsync();

            TempData["SuccessMessage"] = "Pet removed.";
            return RedirectToAction("Profile", new { edit = 1 }); // optional
        }


        public class OwnedPetInputDto
            {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> SaveOwnedPets(string petsJson)
        {
            var user = await _userManager.GetUserAsync(User);
            if (user == null) return RedirectToAction("Login");

            var list = JsonSerializer.Deserialize<List<OwnedPetInputDto>>(petsJson ?? "[]",
                new JsonSerializerOptions { PropertyNameCaseInsensitive = true }
            ) ?? new List<OwnedPetInputDto>();

            // wipe + reinsert (simple)
            var existing = await _db.OwnedPets.Where(p => p.UserId == user.Id).ToListAsync();
            _db.OwnedPets.RemoveRange(existing);

            var toAdd = list.Select(p => new OwnedPet
            {
                UserId = user.Id,
                Name = (p.Name ?? "").Trim(),
                Breed = (p.Breed ?? "").Trim(),
            ...
            _db.OwnedPets.AddRange(toAdd);
            await _db.SaveChangesAsync();

            TempData["SuccessMessage"] = "Current pets updated.";
            return RedirectToAction("Profile");
        }
        ...
        public async Task<IActionResult> CompleteRegistration(HomeLivingViewModel model)
        {
            ...
            if (hasOtherPets)
            {
             ...
             ?? new List<AddPetViewModel>();
                ...
                // If the user added pets, treat it as hasOtherPets
                hasOtherPets = vmPets.Count > 0;

                // update these so DB stays consistent
                user.HasOtherPets = hasOtherPets;
                user.OtherPetsJson = hasOtherPets ? petsJson : "[]";
                await _userManager.UpdateAsync(user);

                // Save to OwnedPets table if there are any
                if (vmPets.Count > 0)
                {
                    var ownedPets = vmPets.Select(p => new OwnedPet
                    {
                        ...
                        Gender = (p.Gender ?? "").Trim(),
                        Breed = (p.Breed ?? "").Trim(),
                        Age = p.Age ?? 0,              // ✅ IMPORTANT (if Age is int?)
                        CreatedAtUtc = DateTime.UtcNow
                    }).ToList();
