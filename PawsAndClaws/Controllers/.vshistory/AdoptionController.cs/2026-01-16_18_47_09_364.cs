...
namespace PawsAndClaws.Controllers
{
    ...
    public class AdoptionController : Controller
    {
        ...
        public async Task<IActionResult> MyApplications(string status = "All")
        {
            ...
            {
                appsQuery = appsQuery.Where(a => !string.Equals(a.Status, "CANCELLED", StringComparison.OrdinalIgnoreCase));
            }
            ...
            {
                appsQuery = appsQuery.Where(a => string.Equals(a.Status, status, StringComparison.OrdinalIgnoreCase));
            }
           
            var filteredApps = await appsQuery.ToListAsync();
        ...
        private static Dictionary<string, int> BuildCounts(IEnumerable<AdoptionApplication> apps)
        {
            ...
            int cancelledCount = list.Count(a => string.Equals(a.Status, "CANCELLED", StringComparison.OrdinalIgnoreCase));
            int nonCancelledCount = list.Count - cancelledCount;

            var dict = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase)
            {
                ["All"] = nonCancelledCount,
