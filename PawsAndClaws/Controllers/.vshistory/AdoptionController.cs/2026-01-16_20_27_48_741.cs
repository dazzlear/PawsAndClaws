...
namespace PawsAndClaws.Controllers
{
    ...
    public class AdoptionController : Controller
    {
        ...
        public async Task<IActionResult> Submit(int petId, string message)
        {
            ...
            var invStatus = (pet.Status ?? "AVAILABLE").Trim().ToUpperInvariant();
            if (invStatus == "PENDING" || invStatus == "ADOPTED")
            {
        ...
        private async Task SyncPetInventoryStatusAsync(int petId)
        {
            // Load pet + all applications for correct recompute
            ...
            var statuses = pet.Applications
                .Select(a => (a.Status ?? "").Trim().ToUpperInvariant())
                .ToList();

            // Priority rules:
            // 1) If ANY application is ADOPTED -> Inventory = Adopted
            if (statuses.Contains("ADOPTED"))
            {
                pet.Status = "Adopted";
            }
            // 2) If ANY application is PENDING or APPROVED -> Inventory = Pending
            else if (statuses.Any(s => s == "PENDING" || s == "APPROVED"))
            {
                pet.Status = "Pending";
            }
            // 3) Otherwise (REVIEW / REJECTED / CANCELLED only) -> Inventory = Available
            else
            {
...
)
        {
            ...
            // Sync to Inventory ONLY for PENDING and ADOPTED
            if (app.Pet != null)
            {
