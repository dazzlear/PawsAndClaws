...
namespace PawsAndClaws.Controllers
{
    ...
    public class AdoptionController : Controller
    {
        ...
        {
            var query = _db.AdoptionApplications
                .AsNoTracking()
                .Include(a => a.Pet)
                .Include(a => a.User);

            // load all first so StatusCounts is correct
            ...
            var viewModel = new ApplicationPageViewModel
            {
                ...
                Applications = filtered
                    ...
                    .Select(a => new ApplicationCardViewModel
                    {
                        ...
                        // same pattern you used in MyApplications:
                        Details = $"{a.Pet?.Species} • {a.Pet?.Age} years • {a.Pet?.Gender}",
                ...
                // keep empty for now (or map from DB later)
