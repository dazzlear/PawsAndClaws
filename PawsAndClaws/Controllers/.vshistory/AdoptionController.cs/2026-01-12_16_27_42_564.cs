...
namespace PawsAndClaws.Controllers
{
    ...
    public class AdoptionController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> UpdateApplicationStatus(UpdateApplicationStatusRequest req)
        {
            ...
            app.Status = req.Status?.Trim() ?? "Pending";

            // Optional: if you want "step" to drive status, or vice versa
            // app.Status = StepToStatus(req.CurrentStep); // only if you prefer

            await _db.SaveChangesAsync();

            TempData["SuccessMessage"] = "Application updated.";
            return RedirectToAction(nameof(ManageApplications));
        }
    }
}